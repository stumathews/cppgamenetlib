...
{
	UdpGameServerConnection::UdpGameServerConnection(const std::string& host, const std::string& port, const gamenetlib::Encoding wireFormat)
	{
		this->host = host;
		this->port = port;
	...
	void UdpGameServerConnection::Initialize()
	{
		...
		this->eventManager = gamelib::EventManager::Get();
	...
	std::vector<std::shared_ptr<gamelib::Event>> UdpGameServerConnection::HandleEvent(const std::shared_ptr<gamelib::Event>& evt, const unsigned long deltaMs)
	{
