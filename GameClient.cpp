...
namespace gamenetlib
{
	...
	{
		gamelib::Logger::Get()->LogThis("Initializing game client...");
		
		eventManager = gamelib::EventManager::Get();
		networking = Networking::Get();
		eventFactory = gamenetlib::EventFactory::Get();
		
		serializationManager = std::make_shared<SerializationManager>(encoding);

		networkProtocolManager->Initialize();

		gamelib::Logger::Get()->LogThis(isTcp 
			                                ? "Client using TCP"
			                                : "Client using UDP");

		gamelib::Logger::Get()->LogThis("Nickname:" + std::string(nickName));
	...
	void GameClient::Connect(const std::shared_ptr<GameServer>& inGameServer)
	{
		...
		gamelib::Logger::Get()->LogThis("Server socket created. Connected to server.");
	...
	{
		gamelib::Logger::Get()->LogThis("Registering client with game server.");
		...
		gamelib::Logger::Get()->LogThis(sendResult == 0 ? "Error: 0 bytes sent." : "client/player details successfully sent.");		
	}
	...
	void GameClient::CheckSocketForTraffic(const unsigned long deltaMs)
	{
		...
		if (FD_ISSET(networkProtocolManager->GetConnection()->GetRawSocket(), &readfds))
		{
			...
			else if (bytesReceived == SOCKET_ERROR && WSAGetLastError() == WSAECONNRESET)
			{
				...
				gamelib::Logger::Get()->LogThis("Connection closed. Server died.");
			}
		}
	}
	...
	void GameClient::ParseReceivedServerPayload(const char* buffer, const unsigned long deltaMs) const
	{
		...
		const bool noTarget = messageType == gamenetlib::StartNetworkLevelEventId.Name;
	...
	std::vector<std::shared_ptr<gamelib::Event>> GameClient::HandleEvent(const std::shared_ptr<gamelib::Event>& evt, const unsigned long deltaMs)
	{
		if (sendClientEventsToServer)
		{
			gamelib::Logger::Get()->LogThis("Sending client event to server:");
			gamelib::Logger::Get()->LogThis(evt->ToString());

			const auto message = serializationManager->SerializeEvent(evt, nickName);
			const int sendResult = InternalSend(message, deltaMs);

			gamelib::Logger::Get()->LogThis(sendResult > 0 
				                                ? "Successfully sent."
				                                : "Error no data sent");
		}
		return {};
