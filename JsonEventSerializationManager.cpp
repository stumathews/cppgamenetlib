...
{
	std::string JsonEventSerializationManager::SerializePlayerMovedEvent(const std::shared_ptr<gamelib::PlayerMovedEvent> object, std::string target)
	{
		const Json payload = Json::object
		{
			{ "messageType", object->Id.Name },
			{ "direction", gamelib::DirectionUtils::ToString(object->Direction) },
			{ "nickname", target }
		};
    ...
    std::shared_ptr<gamelib::PlayerMovedEvent> JsonEventSerializationManager::DeserializePlayerMovedEvent(const std::string serializedMessage)
    {
        std::string error;
        const auto payload = Json::parse(serializedMessage.c_str(), error);
        const auto& direction = payload["direction"].string_value();

        return std::make_shared<gamelib::PlayerMovedEvent>(gamelib::DirectionUtils::FromDirectionString(direction));
    }	
	...
	std::string JsonEventSerializationManager::SerializeControllerMoveEvent(const std::shared_ptr<gamelib::ControllerMoveEvent> object, std::string target)
    {
	    const Json payload = Json::object
		{
			{ "messageType", object->Id.Name },
	...
	std::string JsonEventSerializationManager::CreateUnknownEventMessage(std::shared_ptr<gamelib::Event> evt, std::string target) 
	{
		const Json payload = Json::object
		{
			{ "messageType", "unknown" },
