...
void gamenetlib::ReliableUdpMessageHeader::Write(BitPacker<uint32_t>& bitPacker, const bool includeChecksum = true) const
{
	bitPacker.Pack(16, Sequence);
	bitPacker.Pack(16, LastAckedSequence);
	bitPacker.Pack(32, LastAckedBits);
	bitPacker.Pack(BITS_REQUIRED(0,3), MessageType);
...
void gamenetlib::ReliableUdpMessageHeader::Write(BitPacker<uint32_t>& bitPacker) const
{
	Write(bitPacker, true);
}

void gamenetlib::ReliableUdpMessageHeader::Read(BitfieldReader<uint32_t>& bitfieldReader, const bool includeChecksum = true)
{
...
void gamenetlib::ReliableUdpMessageHeader::Read(BitfieldReader<uint32_t>& bitfieldReader)
{
	Read(bitfieldReader, true);
}
int gamenetlib::ReliableUdpMessageHeader::GetSizeInBits()
{
