...
namespace gamenetlib
{
	ReliableUdpGameServerConnection::ReliableUdpGameServerConnection(const std::string& host, const std::string& port,
	                                                                 ...
	                                                                 const gamenetlib::Encoding wireFormat):
	...
	void ReliableUdpGameServerConnection::CheckForPlayerTraffic(const unsigned long deltaMs)
	{
		...
		{
			gamelib::Logger::Get()->LogThis("Server: Secure session successfully established.");
			...
			*sharedNonce = 1;
			...
			{
				gamelib::Logger::Get()->LogThis("Server: Decrypt failed.");
				return;
			}

			gamelib::Logger::Get()->LogThis("Server: Successfully decrypted message.");
		...
		{
			gamelib::Logger::Get()->LogThis("Server: [Secure session not yet established]");
		}

		if (bytesReceived > 0)
		{
			gamelib::Logger::Get()->LogThis("Server: Received " + std::to_string(bytesReceived) + " bytes");

			// Hook up to a 32-bit bitfield reader to the received buffer. This will read 32-bits at a time			
			gamenetlib::BitfieldReader reader(readBuffer, numReceivedReadBufferElements);
						
			// Unpack receive buffer into message
			gamenetlib::Message message;
			message.Read(reader);

			// Check for any transmission errors
			if(!message.ValidateChecksum())
			{
				gamelib::Logger::Get()->LogThis("Server: Received invalid checksum");

				RaiseEvent(EventFactory::Get()->CreateReliableUdpCheckSumFailedEvent(std::make_shared<gamenetlib::Message>(message)));
			...
			gamelib::Logger::Get()->LogThis("Server: [Received "+ MessageTypeToString(message.Header.MessageType) +"]");				

			if(message.Header.MessageType == gamenetlib::MessageType::Ack) // ack message
			{
				RaiseEvent(EventFactory::Get()->CreateReliableUdpAckPacketEvent(std::make_shared<gamenetlib::Message>(message), false));
				
				RaiseEvent(EventFactory::Get()->CreateReliableUdpPacketRttCalculatedEvent(std::make_shared<gamenetlib::Message>(message),
			...
			{
				RaiseEvent(EventFactory::Get()->CreateReliableUdpPacketReceived(std::make_shared<gamenetlib::Message>(message)));
			...
			if (message.Header.MessageType == SendPubKey)
			{
				...
				gamelib::Logger::Get()->LogThis("Server: Sending public key in response");
	...
	int ReliableUdpGameServerConnection::SendAck(const SOCKET socket, const Message& messageToAck, const int flags, PeerInfo& peerInfo, const unsigned long sendTimeMs)
	{
		...
		gamelib::Logger::Get()->LogThis("Server: Sending ack.");
