...
namespace gamenetlib
{
	...
	public:
		ReliableUdpGameServerConnection(const std::string& host, const std::string& port, bool useEncryption = true,
		                                gamenetlib::Encoding wireFormat = gamenetlib::Encoding::json);
	private:
		...
		int InternalSend(SOCKET socket, const char* buf, int len, int flags, const sockaddr* to, int toLen,
		                 unsigned long sendTimeMs, gamenetlib::MessageType messageType);
		...
		int SendAck(SOCKET socket, const gamenetlib::Message& messageToAck, int flags, PeerInfo& peerInfo, unsigned long sendTimeMs = 0);
