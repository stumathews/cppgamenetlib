	...
	: gameServerConnection(std::move(gameServerConnection)), useEncryption(useEncryption)
{
	...
	void ReliableUdpProtocolManager::Connect(const char* address, const char* port)
	{
		...
		gamelib::Logger::Get()->LogThis("Client: [Connected]");

		if(!sessionEstablished)
		{
			gamelib::Logger::Get()->LogThis("Client: Sending public key to server...");
	...
	int ReliableUdpProtocolManager::Send(const char* callersSendBuffer, const int dataLength, const unsigned long deltaMs)
	{
		if(!sessionEstablished)
		{
			gamelib::Logger::Get()->LogThis("Client: [Detected secure session not yet established]");

			// Do not send the data if the session is not encrypted. Send the public key instead.

			gamelib::Logger::Get()->LogThis("Client: Sending public key...");
	...
	int ReliableUdpProtocolManager::SendInternal(const char* callersSendBuffer, const int dataLength, const unsigned long deltaMs, const MessageType messageType)
	{
		...
		{
			this->RaiseEvent(gamenetlib::EventFactory::Get()->CreateReliableUdpPacketLossDetectedEvent(std::make_shared<Message>(*message)));
		}

		gamelib::Logger::Get()->LogThis("Client: [Sending " + MessageTypeToString(messageType) + "]");
		...
		{
			gamelib::Logger::Get()->LogThis("Client: [Secure session established]");
	...
	int ReliableUdpProtocolManager::ReceiveInternal(char* callersReceiveBuffer, const int bufLength, const unsigned long deltaMs)
	{
		...
		if(bytesReceived < 0)
		{
			gamelib::Logger::Get()->LogThis("Client: No data received!");
			return bytesReceived;
		}

		gamelib::Logger::Get()->LogThis("Client: [Receiving data]");
		...
		if(sessionEstablished && useEncryption)
		{
			gamelib::Logger::Get()->LogThis("Client: [Secure session successfully established]");
			...
			*sharedNonce = 1;
			...
			if(decrptResult < 0)
			{
				gamelib::Logger::Get()->LogThis("Client: [Decrypt failed]");
				return bufLength;
			}
		...
		{
			gamelib::Logger::Get()->LogThis("Client: [Checksum failed]");

			RaiseEvent(gamenetlib::EventFactory::Get()->CreateReliableUdpCheckSumFailedEvent(std::make_shared<Message>(message)));
		...
		{
			gamelib::Logger::Get()->LogThis("Client: [Received server's public key]");
		...
		{
			gamelib::Logger::Get()->LogThis("Client: [Received ack]");
			RaiseEvent(gamenetlib::EventFactory::Get()->CreateReliableUdpAckPacketEvent(std::make_shared<Message>(message), false));
			RaiseEvent(gamenetlib::EventFactory::Get()->CreateReliableUdpPacketRttCalculatedEvent(std::make_shared<Message>(message),
		...
		{
			gamelib::Logger::Get()->LogThis("Client: Sending ack...");

			RaiseEvent(gamenetlib::EventFactory::Get()->CreateReliableUdpPacketReceived(std::make_shared<Message>(message)));
			SendAck(message, deltaMs);
			RaiseEvent(gamenetlib::EventFactory::Get()->CreateReliableUdpAckPacketEvent(std::make_shared<Message>(message), true));
		}
	...
	std::vector<std::shared_ptr<gamelib::Event>> ReliableUdpProtocolManager::HandleEvent(const std::shared_ptr<gamelib::Event>& evt, const unsigned long deltaMs)
	{
		return {};
	}
	...
	{
		return gamelib::EventSubscriber::GetSubscriberId();
	}	
}
