...
namespace gamenetlib
{
	...
	public:
		...
		const gamenetlib::Encoding WireEncoding = Encoding::json;
		...
		void StartNetworkServer(std::shared_ptr<gamenetlib::IGameServerConnection> inGameServerConnection = nullptr)
		{
			...
			ListeningThread = thread([&]()
			{
				...
				auto defaultConnection =
					gamenetlib::GameServerConnectionFactory::Create(false /* UDP */, ServerAddress, ListeningPort,
		...
		void SetUp() override
		{
			SerializationManager = std::make_shared<gamenetlib::SerializationManager>(WireEncoding);
		}
		...
		[[nodiscard]] std::tuple<vector<shared_ptr<gamelib::Event>>, vector<shared_ptr<gamelib::Event>>> PartitionEvents() const
		{
			auto events = gamelib::EventManager::Get()->GetEvents();

			vector<shared_ptr<gamelib::Event>> serverEmittedEvents;
			vector<shared_ptr<gamelib::Event>> clientEmittedEvents;
		...
		[[nodiscard]] shared_ptr<gamelib::Event> FindNetworkTrafficReceivedEvent(std::vector<shared_ptr<gamelib::Event>> events, const std::string requiredMessageType, const std::string& trafficEventIdentifier) const
		{
			const auto result = std::find_if(events.begin(), events.end(), 
		     [&](const shared_ptr<gamelib::Event> & event)
		     {
		         if( event->Id == NetworkTrafficReceivedEventId)
		         {
		             const auto trafficReceivedEvent = gamelib::To<NetworkTrafficReceivedEvent>(event);
		...
		[[nodiscard]] shared_ptr<gamelib::Event> FindEmittedEvent(const std::vector<shared_ptr<gamelib::Event>>& events, const gamelib::EventId& eventId, bool findLast = false) const
		{
			auto pred =  [&](const shared_ptr<gamelib::Event> & event)
		     {
		         if( event->Id == eventId )
		         {
		             return true;
		...
		[[nodiscard]] shared_ptr<NetworkPlayerJoinedEvent> FindPlayerJoinedEvent(const std::vector<shared_ptr<gamelib::Event>>& events, const string& playerNick) const
		{
			const auto result = std::find_if(events.begin(), events.end(), 
		     [&](const shared_ptr<gamelib::Event> & event)
		     {
		         return event->Id == NetworkPlayerJoinedEventId && gamelib::To<NetworkPlayerJoinedEvent>(event)->Player.GetNickName() == playerNick ? true : false;
		     });
			return gamelib::To<NetworkPlayerJoinedEvent>(*result);
		}

		void EnsureNetworkJoinedEventEmittedByServer(const std::vector<shared_ptr<gamelib::Event>>& events, const std::string playerNick) const
		{
			const auto resultEvent = FindEmittedEvent(events, NetworkPlayerJoinedEventId);
			const auto joinedEvent = gamelib::To<NetworkPlayerJoinedEvent>(resultEvent);
		...
		void EnsurePongTrafficReceivedByClient(const std::vector<shared_ptr<gamelib::Event>>& clientEmittedEvents, const std::string& messageIdentifier, const std::string& expectedEventOrigin) const
		{
		...
		void EnsurePlayerPingTrafficReceivedByServer(const std::vector<shared_ptr<gamelib::Event>>& serverEmittedEvents, const std::string& messageIdentifier, const std::string& expectedEventOrigin) const
		{
		...
		void EnsurePlayerJoinedTrafficReceivedByServer(const std::vector<shared_ptr<gamelib::Event>>& serverEmittedEvents, const std::string& messageIdentifier, const std::string& expectedEventOrigin) const
		{
	...
	TEST_F(NetworkingTests, ReliableUdpPacketLossResendTest)
	{
		...
		const int countData1 = ranges::count_if(serverEmittedEvents, [data1](shared_ptr<gamelib::Event> event)
		{
			if(event->Id == NetworkTrafficReceivedEventId)
			{
				const auto trafficEvent = To<NetworkTrafficReceivedEvent>(event);
		...
		const int countData2 = ranges::count_if(serverEmittedEvents, [data2](shared_ptr<gamelib::Event> event)
		{
			if(event->Id == NetworkTrafficReceivedEventId)
			{
				const auto trafficEvent = To<NetworkTrafficReceivedEvent>(event);
		...
		const int countData3 = ranges::count_if(serverEmittedEvents, [data3](shared_ptr<gamelib::Event> event)
		{
			if(event->Id == NetworkTrafficReceivedEventId)
			{
				const auto trafficEvent = To<NetworkTrafficReceivedEvent>(event);
	...
	TEST_F(NetworkingTests, ReliableUdpRoutineConversation)
	{
		...
		std::tuple<vector<shared_ptr<gamelib::Event>>, vector<shared_ptr<gamelib::Event>>> allEvents;
		vector<shared_ptr<gamelib::Event>> serverEmittedEvents;
		vector<shared_ptr<gamelib::Event>> clientEmittedEvents;
