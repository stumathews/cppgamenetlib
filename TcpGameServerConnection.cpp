...
{
	TcpGameServerConnection::TcpGameServerConnection(const std::string& host, const std::string& port, const gamenetlib::Encoding wireFormat)
	{
		this->host = host;
		this->port = port;
	...
	void TcpGameServerConnection::Initialize()
	{
		...
		this->_eventManager = gamelib::EventManager::Get();
	...
	std::vector<std::shared_ptr<gamelib::Event>> TcpGameServerConnection::HandleEvent(const std::shared_ptr<gamelib::Event>& evt, const unsigned long deltaMs)
	{
		return {};
	}
	...
	TcpGameServerConnection::~TcpGameServerConnection()
	{
		...
		{			
			...
			gamelib::Logger::Get()->LogThis("Closed player connection.");
		}
	}
}
